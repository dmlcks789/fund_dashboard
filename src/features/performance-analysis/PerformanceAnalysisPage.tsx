...
const INVESTEE_SOURCE_COL_OFFSET = 2; // 소스 col 1-45 → 프레임 col 3-47 고정 오프셋
const INVESTEE_SECTION_ROW = 5; // 0-indexed = row 6 (섹션 헤더)
const INVESTEE_SUBHEADER_ROW = 6; // 0-indexed = row 7 (서브 헤더)
const INVESTEE_FIXED_SRC_COLS = 45; // source 1~45 = 연락처 + DQ코드 (고정 오프셋 구간)
const TIME_SERIES_SECTION_LABELS = ["매출", "직원수", "기업 인수"] as const;
...
function buildInvesteeColMap(sourceSheet: XLSX.WorkSheet, targetSheet: XLSX.WorkSheet): Map<number, number> {
  ...
  for (const label of TIME_SERIES_SECTION_LABELS) {
    ...
    colMap.set(srcSum, tgtSum); // 합계 컬럼 명시 매핑
